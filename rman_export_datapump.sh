...
#             FULL/SCHEMA       - Used to tell the script whether it is doing a full or schema level export
#             SCHEMA NAME       - Only required if doing a schema level export, indicates what schema to export
...
VERSION=`${SQLPLUS} /nolog << EOT > /dev/null
    connect ${DBADMINID}/${DBADMINPWD};
    set feedback off
    set pause off
    set trimspool on
...
fn_log_vars ()
{
  ...
  echo "DATA-PUMP: ${DATA-PUMP}" | ${LOG_CMD_MAIN}
...
fn_run_export()
{
  ...
  if [ ${VERSION} = '10' ]; then 
	 if [ ${EXP_TYPE} = 'FULL' ]; then
	 ${EXPDP} ${EXPID}/${EXPPWD} DUMPFILE=${EXP_OUT} logfile=${CURR_LOG} FULL=Y DIRECTORY=${DATA_PUMP_DIR} PARALLEL=5
	 VALUE_RET=$?

	 else
	 ${EXPDP} ${EXPID}/${EXPPWD} DUMPFILE=${EXP_OUT} logfile=${CURR_LOG} SCHEMAS=${SCHEMA_NAME} PARALLEL=5
	 VALUE_RET=$?
  ...
  if [ ${VERSION} = '11' ]; then
        if [ ${EXP_TYPE} = 'FULL' ]; then
        ${EXPDP} ${EXPID}/${EXPPWD} DUMPFILE=${EXP_OUT} logfile=${CURR_LOG} FULL=Y DIRECTORY=${DATA_PUMP_DIR} PARALLEL=5 COMPRESSION=ALL
	VALUE_RET=$?

	else
        ${EXPDP} ${EXPID}/${EXPPWD} DUMPFILE=${EXP_OUT} logfile=${CURR_LOG} SCHEMAS=${SCHEMA_NAME} PARALLEL=5 COMPRESSION=ALL 
	VALUE_RET=$?
...
#
  ...
  if [ -z $1 ]; then
    ...
    echo "rman_export.sh <DB_ADMIN_DIR> <DBNAME> <DEV/MOD/PROD> <8/9/10/11> <FULL/SCHEMA> <SCHEMA NAME> <COMPRESS>."
    echo "Aborting!"
    exit 1
  fi
  ...
  if [ -z $4 ]; then
        echo "Oracle version <8/9/10/11>  was not passed correctly."
        echo "Correct format for calling is : "
        echo "rman_export.sh <DB_ADMIN_DIR> <DBNAME> <DEV/MOD/PROD> <8/9/10/11> <FULL/SCHEMA> <SCHEMA NAME> <COMPRESS>."
        echo "Aborting!"
        exit 1
  ...
  if [ -z $5 ]; then
    echo "Export type (FULL/SCHEMA) was not passed correctly."
    echo "Correct format for calling is : "
    echo "rman_export.sh <DB_ADMIN_DIR> <DBNAME> <DEV/MOD/PROD> <8/9/10/11>  <FULL/SCHEMA> <SCHEMA NAME> <COMPRESS>."
    echo "Aborting!"
    exit 1
...
#
#checking datapump directory 
#
     case ${VERSION} in
	10) if [ ${DATA_PUMP_DIR} = 'NONE' ]; then
		echo "ERROR -Datapump directory not mentioned " | ${LOG_CMD_MAIN}
		exit 1
	     fi ;;
	11) if [ ${DATA_PUMP_DIR} = 'NONE' ]; then
		echo "ERROR -Datapump directory not mentioned " | ${LOG_CMD_MAIN}
		exit 1
              fi ;;
     esac 	
...
case ${EXP_TYPE} in
    FULL) OUTF=${LOG_DIR}/call_export_full.sh.out
          ...
          export EXP_FILE=exp_${DBNAME}_${EXP_FILE_DATE}_full_%U.dmp
          export EXP_PIPE=exp_pipe_${DBNAME}_${EXP_FILE_DATE}_full;;
  SCHEMA) OUTF=${LOG_DIR}/call_export_${SCHEMA_NAME}.sh.out
          SCRIPT_NAME=export_${SCHEMA_NAME}
          SCRIPT=call_export_${SCHEMA_NAME}.sh
          export EXP_FILE=exp_${DBNAME}_${EXP_FILE_DATE}_${SCHEMA_NAME}_%U.dmp
          export EXP_PIPE=exp_pipe_${DBNAME}_${EXP_FILE_DATE}_${SCHEMA_NAME};;
esac

{
  ...
  CURR_LOG=${SCRIPT_NAME}_${DBNAME}_${EXP_FILE_DATE}.log
